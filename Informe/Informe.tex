...
\begin{titlepage}
    ...
    \includegraphics[scale=0.9]{figuras/descarga.jpg}\par\vspace{1cm}
...
\begin{itemize}
    \item \textbf{users}: información básica del usuario.
        \begin{itemize}
            ...
            \item \texttt{name} TEXT NOT NULL
            \item \texttt{pin} TEXT NOT NULL              % hash bcrypt del PIN
            \item \texttt{active} INTEGER DEFAULT 1
...
\begin{enumerate}
    \item Captura de al menos 10 fotogramas con calidad mínima de detección
    \item Validación de cada fotograma para evitar imágenes borrosas o parcialmente ocluidas
    \item Generación de embeddings faciales usando el modelo ResNet50 de DeepFace
...
\begin{enumerate}
    \item Captura continua del stream de video
    \item Detección de rostros en cada fotograma utilizando MTCNN (Multi-task Cascaded Convolutional Networks)
    \item Extracción de embedding facial del rostro detectado
    \item Comparación con embeddings almacenados usando distancia euclidiana
    \item Aplicación de threshold de similitud (configurado en 0.7 en el archivo config.py)
    \item Generación de log con timestamp, usuario identificado y grado de confianza
    \item Activación de mecanismo de acceso si la similitud supera el threshold
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Reducción de Resolución:} Procesamiento de fotogramas a 320x240 píxeles 
    para detectar rostros, manteniendo precisión
    
    \item \textbf{Caché de Embeddings:} Almacenamiento en memoria de características faciales 
    para reducir accesos a base de datos
    
    \item \textbf{Procesamiento Asincrónico:} Utilización de threads para evitar bloqueos 
    en la interfaz gráfica
    
    \item \textbf{Detección Selectiva:} Análisis solo cuando se detecta movimiento 
    significativo en la imagen
    
    \item \textbf{Modelos Ligeros:} Selección de DeepFace con modelo VGGFace2 
    como balance entre precisión y velocidad
\end{itemize}

\subsubsection{Gestión de Errores y Casos Especiales}

El módulo implementa manejo robusto de situaciones problemáticas:

\begin{itemize}
    \item \textbf{Rostro No Detectado:} Reintentos automáticos durante 5 segundos
    \item \textbf{Múltiples Rostros:} Selección del rostro más prominente o rechazo si la ambigüedad 
    es alta
    \item \textbf{Oclusión Parcial:} Evaluación de calidad antes de procesar
    \item \textbf{Problemas de Iluminación:} Ajuste automático de contraste y brillo
    \item \textbf{Fallo de Cámara:} Mensaje de error y logging de incidente
\end{itemize}
...
\texttt{https://github.com/AlvaroVP96/Proyecto-LANAI.git} % Sustituir por la URL real del repositorio
